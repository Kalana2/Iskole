...
class TimeTableModel
{
    ...
    {
        // Schema uses 'graade' column, not 'grade'
        $sql = "SELECT DISTINCT graade as grade FROM `class` ORDER BY graade";
    ...
    {
        if ($grade !== null && $grade !== '') {
            $sql = "SELECT class FROM `class` WHERE graade = :grade ORDER BY class";
            $stmt = $this->pdo->prepare($sql);
