...
$teachers = [
    ...
    ['id' => 106, 'emp_no' => 'T-2024-0006', 'name' => 'Isuri Mendis', 'subject' => 'IT', 'status' => 'leave', 'attendance_rate' => 90],
];
...
<script>
    function markTeacherStatus(id, status) {
        ...
        input.value = status; row.dataset.status = status;
        badge.className = `status-badge status-${status}`; badge.textContent = status.charAt(0).toUpperCase() + status.slice(1);
        buttons.forEach(b => b.classList.remove('active'));
        const target = row.querySelector(`.btn-${status}`); if (target) target.classList.add('active');
        updateTeacherStats();
    }
    function markAllTeachers(status) { if (confirm(`Mark all teachers as ${status}?`)) { document.querySelectorAll('.teacher-row').forEach(r => { markTeacherStatus(r.dataset.teacherId, status); }); } }
    function resetAttendance() { if (confirm('Reset all changes?')) location.reload(); }
    function updateTeacherStats() {
        let present = 0, absent = 0, leave = 0; const rows = document.querySelectorAll('.teacher-row');
        rows.forEach(r => { const s = r.dataset.status; if (s === 'present') present++; else if (s === 'absent') absent++; else if (s === 'leave') leave++; });
        const total = rows.length; const pct = total ? Math.round((present / total) * 100) : 0;
    ...
    function cancelAttendance() { if (confirm('Cancel attendance? Changes will be lost.')) window.history.back(); }
    // Submit
    document.getElementById('teacherAttendanceForm')?.addEventListener('submit', function (e) { e.preventDefault(); const fd = new FormData(this); const data = {}; for (const [k, v] of fd.entries()) { if (k.startsWith('attendance[')) { const id = k.match(/\d+/)[0]; data[id] = v; } } console.log('Submitting teacher attendance', data); alert('Teacher attendance submitted (demo).'); });
    // Date validation
    document.getElementById('date')?.addEventListener('change', function () { const sel = new Date(this.value); const t = new Date(); t.setHours(0, 0, 0, 0); if (sel > t) { alert('Future dates not allowed'); this.value = '<?php echo $today; ?>'; } });
</script>